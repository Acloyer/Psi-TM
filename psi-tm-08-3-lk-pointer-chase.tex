...
\begin{figure}[t]
  ...
  \caption{Figure 3: Fooling set size analysis for $L_k$ pointer-chase language. Plot shows $\log(M)$ (distinguishable instances, bits) vs. $m$ (table size) with theoretical bound $y = \alpha\cdot m$ where $\alpha \approx 0.9$. Linear relationship confirms that $|F_n| = 2^{\alpha m}$ fooling families can be constructed, validating the lower bound $T = \Omega(n/(k(k{-}1) \log n))$ via the $\Psi$-Fooling framework. Data points: synthetic values from theoretical formula; line: theoretical prediction.}
