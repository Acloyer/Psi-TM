...
\begin{figure}[t]
  ...
  \caption{Fooling set size analysis for $L_{k}$ pointer-chase language. Plot shows $\log(M)$ (distinguishable instances, bits) vs. $m$ (table size) with theoretical bound $y = \alpha\cdot m$ where $\alpha \approx 0.9$. Linear relationship confirms that $\lvert\mathcal{F}_{n}\rvert = 2^{\alpha m}$ fooling families can be constructed, validating the lower bound $T = \Omega\!\big(n/(k(k{-}1)\log n)\big)$ via $\Psi$-Fooling framework. Data points: empirical measurements; line: theoretical prediction.}
