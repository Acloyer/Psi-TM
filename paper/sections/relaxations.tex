...
\setlength{\tabcolsep}{4.5pt} % tighter horizontal padding
\renewcommand{\arraystretch}{1.05} % slightly tighter vertical spacing
...
\paragraph{Lemma (R1: entropy budget).}\label{lem:R1-entropy-budget}\;{\small\texttt{(lem:R1-entropy-budget)}}
...
\paragraph{Lemma (R2: multi-pass).}\label{lem:R2-multipass}\;{\small\texttt{(lem:R2-multipass)}}
...
\paragraph{Lemma (R3: advice).}\label{lem:R3-advice}\;{\small\texttt{(lem:R3-advice)}}
...
\paragraph{Lemma (R4: bandwidth tweak).}\label{lem:R4-bandwidth-tweak}\;{\small\texttt{(lem:R4-bandwidth-tweak)}}
