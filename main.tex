...
\item \textbf{Largeness:} $\mathcal{P}$ holds for a large fraction of functions:
   ...
   \textbf{Formal Proof of Largeness:}
   Let $S_n$ be the set of functions $f: \{0,1\}^n \to \{0,1\}$ with simple structural patterns. We prove $|S_n| \geq 2^{2^n - O(n)}$:
   \begin{enumerate}
   \item A function has simple structural patterns if its truth table can be compressed by a factor of $O(n)$
   \item The number of compressible functions is at least $2^{2^n - O(n)}$ by counting arguments
   \item Each compressible function satisfies $\mathcal{P}$ by definition
...
\textbf{Step 3: Polynomial Interpolation}
We use specific diagonalization structure:
\begin{itemize}
\item For each machine $M$, constraint: $p(x_M) = 1 \iff M(x_M) = 0$
\item These constraints have special algebraic structure
\item By degree bounds for diagonalization polynomials, degree $\geq 2^{\Omega(n)}$ required
\end{itemize}
...
\textbf{Step 2: Frege Lower Bound}
Formal Lower Bound Proof:
\begin{enumerate}
\item Tautology $\tau_n$ requires proving $M(\langle M \rangle) = 0$ for all $M$
\item Number of machines: $2^{\Omega(n)}$
\item Each machine needs separate diagonalization argument
\item By [Razborov-Rudich] bounds for universal statements:
   Frege proof size $\geq n^{\Omega(\log n)}$
\item But Psi-proof uses $\texttt{INT\_STRUCT(2)}$: size $O(n^2)$
\end{enumerate}
