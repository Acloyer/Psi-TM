...
\begin{definition}[k-Level Introspection API]
The introspection API at level $k$ provides access to:
\begin{itemize}
\item $\texttt{INT\_STATE()}$: Current state (depth 1)
\item $\texttt{INT\_CODE(i)}$: Code symbol at position $i$ (depth 1)
\item $\texttt{INT\_INPUT(j)}$: Input symbol at position $j$ (depth 1)
\item $\texttt{INT\_STRUCT(d)}$: Structural patterns at depth $d \leq k$
\item $\texttt{INT\_PATTERN(k)}$: Complete structural pattern of depth $k$
\end{itemize}
\end{definition}
...
\item On input $w = \text{encode}(T)\#1^n$:
  \begin{itemize}
  \item If $T$ has depth $k$: Return actual depth-$k$ patterns via $\texttt{INT\_PATTERN(k)}$
  \item If $T$ has depth $k+1$: Return only the depth-$k$ projection via $\texttt{INT\_PATTERN(k)}$
  \end{itemize}
\item The adversary ensures that $\iota_k(w_1, \varepsilon, k) = \iota_k(w_2, \varepsilon, k)$ for any two inputs $w_1, w_2$ where one has depth $k$ and the other has depth $k+1$
\end{enumerate}
...
\item Therefore, $\iota_k(\text{encode}(T_1), \varepsilon, k) = \iota_k(\text{encode}(T_2), \varepsilon, k)$
