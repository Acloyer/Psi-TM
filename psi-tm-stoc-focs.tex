...
\begin{abstract}
We establish an oracle-relative strict hierarchy based on introspection depth: for each $k \geq 1$, there exists an oracle and a language $L_k$ such that $L_k \in \text{Psi-P}_{k+1}$ but $L_k \notin \text{Psi-P}_k$ relative to that oracle. This demonstrates strictly increasing power with $k$ in the oracle setting. Non-oracle strictness remains open. We provide explicit oracle constructions and information-theoretic lower bounds supporting these separations.
\end{abstract}
